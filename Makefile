...
CPPFLAGS = -g -Wall -Wextra -std=c++11 -fno-common
LDFLAGS =
...
COMMONTARGETNAME = doipcommon

COMMONTARGET = lib$(COMMONTARGETNAME)
...
all: env $(BUILDPATH)/$(CLIENTTARGET).a
...
$(BUILDPATH)/%.o: $(COMMONTARGET)/$(SRCPATH)/%.cpp
	$(CXX) $(CPPFLAGS) -I $(COMMONTARGET)/$(INCPATH) -c $< -o $@
	
$(BUILDPATH)/%.o: $(SERVERTARGET)/$(SRCPATH)/%.cpp
	$(CXX) $(CPPFLAGS) -I $(SERVERTARGET)/$(INCPATH) -I $(COMMONTARGET)/$(INCPATH) -c $< -o $@
	
$(BUILDPATH)/%.o: $(CLIENTTARGET)/$(SRCPATH)/%.cpp
	$(CXX) $(CPPFLAGS) -I $(CLIENTTARGET)/$(INCPATH) -I $(COMMONTARGET)/$(INCPATH) -c $< -o $@
    
#$(BUILDPATH)/$(COMMONTARGET).a: $(COMMONOBJS)
#	$(CXX) $(CPPFLAGS) $^ $(LDFLAGS) -o $@
	
#$(BUILDPATH)/$(SERVERTARGET).a: $(SERVEROBJS)
#	$(CXX) $(CPPFLAGS) $^ $(LDFLAGS) -o $@
	
#$(BUILDPATH)/$(CLIENTTARGET).a: $(CLIENTOBJS) $(BUILDPATH)/$(COMMONTARGET).a
#	$(CXX) $(CPPFLAGS) $^ $(LDFLAGS) -L./$(BUILDPATH)/ -l$(COMMONTARGETNAME) -o $@


#$(BUILDPATH)/$(COMMONTARGET).a: $(COMMONOBJS)
#	$(AR) $(ARFLAGS) $@ $^ 
	
$(BUILDPATH)/$(CLIENTTARGET).a: $(CLIENTOBJS) $(COMMONOBJS)
	$(AR) $(ARFLAGS) $@ $^
